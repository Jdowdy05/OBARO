...
def XML_store_data(objects_tree : Element, n : int, name_list : "list[str]", object_error : float, background_error : float, object_surface_brightness : float,
    ...
    ET.SubElement(dataField, 'File_Name', name = str(name_list)+":"+str(object_number)).text
...
def XML_store_data(objects_tree : Element, n : int, name_list : "list[str]", object_error : float, background_error : float, object_surface_brightness : float,
    ...
    objects_tree.append(dataField)
    ET.SubElement(dataField, 'File_Name', Name = str(name_list)+":"+str(object_number)).text
    ET.SubElement(dataField, 'Object_Mean_Pixel_Value', object_mean = str(object_mean)).text
    ET.SubElement(dataField, "Object_Total_Pixel_Value_Error", object_error = str(object_error)).text
    ET.SubElement(dataField, "Object_Region_Area", object_area = str(object_area)).text
    ...
    ET.SubElement(dataField, "Background_Surface_Brightness", background_surface_brightness = str(background_surface_brightness)).text
    ET.SubElement(dataField, "Background_Surface_Brightness_Error", background_surface_brightness_error =  str(background_surface_error)).text
    ET.SubElement(dataField, "Background_Pixel_Count", background_pixel_count = str(background_pixel_count)).text
...
def write_XML(tree : Element, xml_name : str) -> None:
    ...
    tree_xml.write(xml_name+'.xml', encoding='utf-8', xml_declaration=True)
...
def parse_XML(XMLname : str) -> np.ndarray:
    ...
    try:
        for child in root:
            object_mean.append(float(root[i][4].attrib['object_surface_brightness']))
            background_mean.append(float(root[i][10].attrib['background_surface_brightness']))
            i=i+1
    except:

        for child in root:
            object_mean.append(float(root[i][4].attrib['value']))
            background_mean.append(float(root[i][10].attrib['value']))
            i=i+1
